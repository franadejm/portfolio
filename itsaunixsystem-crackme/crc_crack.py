...
table = bytes([
  ...
  , 
  ...
  0xD1, 0xAE, 0x35, 0xF1, 0xAD, 0x45, 0x66, 0x0B, 0xDF, 0x40, 
  ...
  0x8D, 0xEF, 0x02, 0x2D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
  0x00, 0x00
])
...
def hash_func(data: bytes) -> int:
    ...
    for b in data:
        c = (b ^ h) & 0xFF
        h >>= 8
        table_val = struct.unpack_from("<I", table, 4 * c)[0]
        h ^= table_val
    return (~h) & 0xFFFFFFFF
...
def brute_force(max_len=8):
    for length in range(1, max_len + 1):
        print(f"[*] Trying length {length} (including newline)")
        for prefix in itertools.product(PRINTABLE, repeat=length - 1):
            data = bytes(prefix + (NEWLINE,))
            if hash_func(data) == TARGET:
                print("[+] FOUND!")
                print("String:", data.decode("ascii", errors="replace"))
                print("Hex   :", data.hex())
                return
    print("[-] Not found")

if __name__ == "__main__":
    brute_force(max_len=8)
